...
e1x=np.amin(e1)
e1y=np.amax(e1)
gg1,xedges=np.histogram(e1, bins=10,range=[-2.4,-0.8],density=True)
...
xs = np.linspace(smean-10*sstd,smean+10*sstd,1000)
density.covariance_factor = lambda :0.50
...
plt.xticks(np.arange(-100,100,0.4),size=10)
plt.yticks(size=10)
plt.xlim(-2.4,-0.8)
#plt.ylim(0,4.2)
...
e1x=np.amin(e1)
e1y=np.amax(e1)
gg1,xedges=np.histogram(e1, bins=8,range=[-2.4,-0.8],density=True)
xcenters = (xedges[:-1] + xedges[1:]) / 2
