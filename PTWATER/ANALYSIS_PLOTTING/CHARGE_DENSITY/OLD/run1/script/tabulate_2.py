...
const3=6.241506363094*10**(18)*3.33564*10**(-30)*-1.60217663*10**(-19)/(10.8*8.854*10**(-12)*10**(-20))
...
###
    for j in range(1664-56,1664):
        t=a[j+i*1664].split()
        a1.append(t)
    for j in range(1664-224,1664-224+56):
        t=a[j+i*1664].split()
        b1.append(t)
    d1=[]
...
##############################################################################################
    ...
    ddip1.append(const6*sssum1)#dipole 
    ddip2.append(const6*sssum2) 
    ddip3.append(const6*sssum3)
    ddip4.append(const6*sssum4)
...
e1x=np.amin(e1)
e1y=np.amax(e1)
gg1,xedges=np.histogram(e1, bins=5,range=[e1x,e1y],density=True)
xcenters = (xedges[:-1] + xedges[1:]) / 2
plt.plot(xcenters,gg1, color='blue',label='clean: QMMM')
#
e1=wwf3
e1x=np.amin(e1)
e1y=np.amax(e1)
gg1,xedges=np.histogram(e1, bins=5,range=[e1x,e1y],density=True)
xcenters = (xedges[:-1] + xedges[1:]) / 2
plt.plot(xcenters,gg1, color='darkorange',label='clean: QMMM+PIGLET')
#
e1=wwf2
e1x=np.amin(e1)
e1y=np.amax(e1)
gg1,xedges=np.histogram(e1, bins=5,range=[e1x,e1y],density=True)
xcenters = (xedges[:-1] + xedges[1:]) / 2
plt.plot(xcenters,gg1, color='red',label='0.5 ML: QMMM')
#
e1=wwf4
e1x=np.amin(e1)
e1y=np.amax(e1)
gg1,xedges=np.histogram(e1, bins=5,range=[e1x,e1y],density=True)
xcenters = (xedges[:-1] + xedges[1:]) / 2
plt.plot(xcenters,gg1, color='green',label='0.5 ML: QMMM+PIGLET')
...
plt.savefig("photo2.jpg", dpi=300, bbox_inches = 'tight',    pad_inches = 0.1)
plt.savefig('photo2.pdf')
###############################################################################################################
