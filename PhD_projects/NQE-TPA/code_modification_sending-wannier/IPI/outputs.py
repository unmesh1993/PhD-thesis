...
class TrajectoryOutput(BaseOutput):
    ...
    def write_traj(self, data, what, stream, b=0, format="xyz", dimension="", units="automatic", cell_units="automatic", flush=True):
        ...
        if key in ["extras"]:
            ...
            x=data[b].split(',')
            ggg = int(self.system.beads.natoms)
            hhh = int((len(x) - ggg*3 - 6)/5)
            for i in range(0,ggg*3,3):
                stream.write(x[i])
                stream.write("\t")
                stream.write(x[i+1])
                stream.write("\t")
                stream.write(x[i+2])
                stream.write("\n")
            for i in range(ggg*3,ggg*3+hhh*5,5):
                stream.write(x[i])
                stream.write("\t")
                stream.write(x[i+1])
                stream.write("\t")
                ...
                stream.write(x[i+4])
                stream.write("\n")
            for i in range(ggg*3+hhh*5,ggg*3+hhh*5+3):
                stream.write(x[i])
                stream.write("\t")
            stream.write("\n")
            for i in range(ggg*3+hhh*5+3,ggg*3+hhh*5+6):
                stream.write(x[i])
                stream.write("\t")
