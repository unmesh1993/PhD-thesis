...
for i in range(frames):
       ...
       loopindex1=np.arange(1,1440,3)
       loopindex2=np.arange(2,1440,3)
       loopindex3=np.arange(1440,1440+hyd,1)
       loopindex=np.concatenate((loopindex1,loopindex2,loopindex3))
       print (np.shape(cord)) 
       print (np.shape(loopindex))
       for j in loopindex:
           sam=np.array(cord[j],float)
           a1=sam
           hhh1=np.zeros(480,float) 
           hhh3=np.zeros((480,3),float)
           ...
           angle1=anglem2(ss1,a1,ss2)

           f=open(filename,'a')
           f.write("%.3f  \t %.3f  \t %.3f \t  %.3f \t  %.3f  \n" %(a1[2],z1,z2,angle1,cnn))
           f.close()
