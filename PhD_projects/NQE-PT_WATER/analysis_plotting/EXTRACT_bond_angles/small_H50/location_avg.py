...
fframes=int(len(aa)/(atoms+2))

avgg1=np.zeros((cut,binn))
avgg2=np.zeros((cut,binn))
avgg3=np.zeros((cut,binn))
avgg4=np.zeros((cut,binn))
avggg1=np.zeros((cut,binn))
avggg2=np.zeros((cut,binn))
avggg3=np.zeros((cut,binn))
avggg4=np.zeros((cut,binn))
avdens=np.zeros((cut,binn))


for sss in range(cut):

    frames=int(fframes/cut)
    wowx=[]
    wowy=[]
    ...
    avgg1[sss],avgg2[sss],avgg3[sss],avgg4[sss]=gg1,gg2,gg3,gg4
    avggg1[sss],avggg2[sss],avggg3[sss],avggg4[sss]=ggg1,ggg2,ggg3,ggg4
    avdens[sss]=dens
 

gg1,gg2,gg3,gg4=np.mean(avgg1,axis=0),np.mean(avgg2,axis=0),np.mean(avgg3,axis=0),np.mean(avgg4,axis=0)
ggg1,ggg2,ggg3,ggg4=np.mean(avggg1,axis=0),np.mean(avggg2,axis=0),np.mean(avggg3,axis=0),np.mean(avggg4,axis=0)
dens=np.mean(avdens,axis=0)
