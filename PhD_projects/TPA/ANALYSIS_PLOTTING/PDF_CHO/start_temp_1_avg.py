...
for i in range(1,tot+1):
    ...
    if  i==1:
        plt.plot(xcenters,gg-np.amin(gg), '-ob', markersize=4, lw=2  ,mfc='white',label=sec[i-1]+' :BOMD')
    elif  i==2:
        plt.plot(xcenters,gg-np.amin(gg), '--Xb', markersize=4, lw=2  ,mfc='white',label=sec[i-1]+' :BOMD')
...
for i in range(1,tot+1):
    ...
    if  i==1:
        plt.plot(xcenters,gg-np.amin(gg), '-or', markersize=4, lw=2  ,mfc='white',label='PIGLET')
    elif  i==2:
        plt.plot(xcenters,gg-np.amin(gg), '--Xr', markersize=4, lw=2  ,mfc='white',label='PIGLET')

    plt.ylabel('prob. density',size=12)
    plt.xlabel(r'$d_{C-O}$ (Å)',size=12)
    plt.legend(prop={'size': 6},loc=0,ncol=2) #bbox_to_anchor=(0.75,1.0))
    plt.xticks(np.arange(2.0,5.0,0.1),size=10)
    plt.yticks(size=10)
    plt.xlim(3.0,3.8)
#    plt.ylim(-2,100)
