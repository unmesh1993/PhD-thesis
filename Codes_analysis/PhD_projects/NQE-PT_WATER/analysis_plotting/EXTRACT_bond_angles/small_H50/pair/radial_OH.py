...
#########################################################################
    ...
    for j in range(2+660,atoms+2,1):
       t1=aa[j+(i)*(atoms+2)].split()
       t2=np.array(t1[1:],float)
       t2[2]=t2[2]-mean
       ...
       if (float(t2[2])) > 2.2 and (float(t2[2])) > zzmin and (float(t2[2])) <= zzmax : 
           coord4.append(t2)


    sam1=np.array(coord1,float)
    sam2=np.array(coord2,float)
    sam12=np.array(coord3,float)
    sam13=np.array(coord4,float)
...
#########################################################################
    ...
    mo1[i,:]=mo1[i,:]*((vec1[0][0]*vec1[1][1]*h)/(len(sam1)*(len(sam12))))
    mo2[i,:]=mo2[i,:]*((vec1[0][0]*vec1[1][1]*h)/(len(sam1)*(len(sam13))))
